...
class Sample:
    ...
    def init(self):
        try:
            category = redis_client.get_rules("match:sample")
            if category:
                self.categories = category
            else:
                logger.warning("⚠️ Redis 中无样品分类数据")
        except Exception as e:
            logger.error(f"❌ 从 Redis 加载样品分类失败: {e}")
    ...
    def get_prompt_section(self) -> str:
        ...
        return f"""
            ## 样品分类规则：
                请根据聊天内容，从以下分类中选择最匹配的样品一级分类（p_cat）和二级分类（s_cat）：
    
                {self.get_categories_text()}
    
            ### 分类原则：
            1. 如果聊天内容明确提到了样品名称，根据样品名称选择分类
            2. 如果提到检测项目或测试标准，根据检测对象选择分类
            3. 如果无法确定，返回 "待确定"
            4. 一级分类和二级分类都必须从上面的列表中选择
            5. 如果都不匹配，一级分类选"其他"，二级分类选"其他"
    ...
    def parse_llm_result(self, llm_result: Dict) -> Dict:
        ...
        """
        sample_classification = llm_result.get("sample", {})

        if not sample_classification:
            return {
                "p_cat": "未知",
                "s_cat": "未知",
                "reason": "大模型未返回分类结果",
        ...
        p_cat = sample_classification.get("p_cat", "")
        s_cat = sample_classification.get("s_cat", "")
        reason = sample_classification.get("reason", "")
        confidence = sample_classification.get("confidence", 0.0)
